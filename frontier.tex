...
For an SSG $G$ let $N(G)$ be obtained by replacing each edge $(v,u)$ with
$v$ controlled by the two edges $v\to c_{v,u}$ and
$c_{v,u}\to u$, $c_{v,u}\to u$, where $c_{v,u}$ is a fresh average
vertex with both successors equal to $u$. Then
\begin{enumerate}[label=(\roman*),leftmargin=2.2em]
\item $|V(N(G))|=|V(G)|+2|\Vmax\cup\Vmin|\le 3N$, and in $N(G)$ no
...
\item $\Vmax(N(G))=\Vmax(G)$, and under the bijection
      $\sigma\mapsto\tilde\sigma$, $\tilde\sigma(v):=c_{v,\sigma(v)}$, one
      has $\val^{N(G)}_{\tilde\sigma}=\val^{G}_\sigma$ on $V(G)$ and
      $\val^{N(G)}_{\tilde\sigma}(c_{v,u})=\val^{G}_\sigma(u)$; likewise
      $\val^{N(G)}=\val^{G}$ on $V(G)$;
\item $S_{\tilde\sigma}=S_\sigma$ for every $\sigma$; hence the
      all-switches runs from $\sigma_0$ in $G$ and from $\tilde\sigma_0$
      in $N(G)$ have the same length and pass through corresponding
      strategies.
\end{enumerate}
\end{lemma}

\begin{proof}
(i) is immediate: every successor of a controlled vertex of $N(G)$ is one
of the new average vertices.

(ii) Traps correspond. If $U$ is a trap of $N(G)$ then $c_{v,u}\in U$
forces $u\in U$, both successors of $c_{v,u}$ being $u$; and $v\in U$
controlled forces some $c_{v,u}\in U$, hence some successor $u$ of $v$ in
$G$ to lie in $U$. So $U\cap V(G)$ is a trap of $G$, and it is nonempty
whenever $U$ is, since a trap of $N(G)$ containing $c_{v,u}$ contains
$u$. Conversely a trap $U$ of $G$ extends to the trap
$U\cup\{c_{v,u}:u\in U\}$ of $N(G)$. Now apply \Cref{lem:trapchar}.

(iii) For $y\in[0,1]^{V(G)}$ let $\hat y$ be its extension with
$\hat y(c_{v,u}):=y(u)$. Then
$T^{N(G)}\hat y=\widehat{T^{G}y}$: at $c_{v,u}$ the average of two copies
of $y(u)$ is $y(u)$; at a controlled $v$ the maximum or minimum over
$\{\hat y(c_{v,v^{(0)}}),\hat y(c_{v,v^{(1)}})\}$ equals that over
$\{y(v^{(0)}),y(v^{(1)})\}$; at an average vertex of $G$ nothing changed.
So $y\mapsto\hat y$ is a bijection between the fixed points of $T^G$ and
those of $T^{N(G)}$, and the same computation applies to $T_\sigma$ and
$T_{\tilde\sigma}$. For stopping $G$ these fixed points are unique
(\Cref{thm:contraction}), which gives the claim; in general one uses
instead that the bijection preserves the pre-fixed points in both
directions and hence the least fixed points, and applies
\Cref{thm:lfp-general}.

(iv) By (iii), $\val_{\tilde\sigma}(c_{v,u})=\val_\sigma(u)$, so the test
$\val_{\tilde\sigma}(\overline{\tilde\sigma}(v))>\val_{\tilde\sigma}(\tilde\sigma(v))$
defining $S_{\tilde\sigma}$ reads
$\val_\sigma(\bar\sigma(v))>\val_\sigma(\sigma(v))$, which defines
$S_\sigma$. Since all-switches replaces $\sigma$ by $\sigma[S_\sigma]$
and the correspondence commutes with that, the two runs are in step.
\end{proof}
...
strategy, then the all-switches rule halts within $p(3N)$ rounds on every
...
$C=\Vmax\cup\Vmin$ carries no edges, so \Cref{def:residue} gives
...
stopping with at most $3N$ vertices and no controlled vertex has a
...
within $p(3N)$ rounds. By \Cref{lem:normalform}(iv) the all-switches run
...
that structure can be deleted at a cost of $2N$ extra average vertices,
each of them a vertex with two parallel edges, without changing a single
value. What the residue rule exploits is therefore not a property of the
game but a property of the presentation, and the same applies to any rule
that resolves the deterministic part optimally --- by attractor sweeps,
by condensation into strongly connected components, or by any other
means. This is why the barrier is stated for the whole class
\Cref{def:residue-blind} rather than for one algorithm. It also isolates
what a successful rule must do: it must remain fast on games in which
$\Vmax\cup\Vmin$ is an independent set, so that the only structure left
to exploit is the interaction of the average vertices --- which is
precisely where \Cref{thm:switch-count} places the difficulty, and
precisely what \Cref{thm:matching-barrier} and \Cref{thm:slack-barrier}
show to be hard for comparison mechanisms as well.
\end{remark}

The normalisation raises $a$, and one may ask whether the parameter is
therefore worthless. It is not: the vertices the normalisation adds are
exactly of the kind that a value-preserving reduction removes again.
...
\begin{remark}\label{rem:splice-invariance}
The normalisation of \Cref{lem:normalform} adds $2|\Vmax\cup\Vmin|$
average vertices, and \emph{every one of them is a splice redex}: by
construction $c_{v,u}$ has both successors equal to $u$. Splicing all of
them returns $G$ itself. Moreover the normalisation does not touch the
out-edges of any average vertex of $G$, so it creates no new redex among
the old vertices and destroys none. A bound stated in terms of the number
of average vertices that survive exhaustive reduction --- for instance
\Cref{thm:few-avg}, which may be run on the reduced game and its values
transported back by \Cref{lem:splice} --- is therefore unaffected by the
normalisation, whereas a bound stated in terms of $a$ itself is not.
This is the precise sense in which $a$ is a parameter of the
presentation and its reduced value is a parameter of the game.
\end{remark}


\begin{remark}\label{rem:normalform-scope}
\Cref{lem:normalform} is also a normalisation of independent interest: at
the cost of a factor $3$ in the number of vertices one may always assume
that $\Vmax\cup\Vmin$ is an independent set of the digraph. All of
\Cref{prob:main} is therefore already present in that subclass. Note
that the normalisation raises $a=|\Vavg|$ from $a$ to $a+2|\Vmax\cup\Vmin|$,
so it must not be applied before invoking a bound that depends on $a$,
such as \Cref{thm:few-avg} or \Cref{thm:switch-count}.
\end{remark}



\begin{remark}[Exact scope of the barrier]\label{rem:barrier-scope}
\Cref{thm:normalform-barrier} quantifies over rules that read the graph
they are given, and that restriction is not cosmetic. Every vertex
$N(G)$ adds is a splice redex, so a rule may legitimately begin by
applying the reductions of \Cref{lem:splice} exhaustively --- a
polynomial-time, value-preserving preprocessing --- and thereby recover
$G$ from $N(G)$. Such a composite rule is fast on $N(G)$, and it is not
residue-blind: on the reduced game the controlled vertices are no longer
an independent set, so \Cref{def:residue-blind} does not constrain it.

The content of the barrier is therefore exactly this, and no more: the
advantage the residue rule holds over all-switches lives entirely in
splice-reducible structure, and is measured honestly only on reduced
games. What remains open, and is the sharp question this subsection
leaves behind, is whether the residue rule --- or any $\rho_B$ --- runs
in polynomially many rounds on stopping games carrying no splice or merge
redex. Nothing proved here bears on that: \Cref{thm:ladder}'s $L_n$ is
reduced and \Cref{prop:residue-ladder} solves it in one round, and we
know of no reduced family on which any $\rho_B$ is slow. Neither do we
have a lower bound, and by \Cref{prop:freeze-escapes} the counting
barriers of this section do not supply one, since $\rho_B$ is not a
switching rule.
\end{remark}
...
Let $G^{\dagger}$ be the stopping SSG on vertices $0,\dots,7$ with
$t_0=8$, $t_1=9$, $\Vmax=\{0,1\}$, $\Vmin=\emptyset$,
$\Vavg=\{2,\dots,7\}$ and
\[
  0\to(4,5),\quad 1\to(6,7),\quad 2\to(3,t_1),\quad 3\to(t_1,t_0),
\]
\[
  4\to(3,3),\quad 5\to(1,1),\quad 6\to(t_0,t_0),\quad 7\to(2,2).
\]
No controlled vertex of $G^{\dagger}$ has a controlled successor. At
$\sigma=(0\mapsto4,\ 1\mapsto6)$ one has
$\val_\sigma=(\tfrac12,0,\tfrac34,\tfrac12,\tfrac12,0,0,\tfrac34)$ and
$S_\sigma=\{1\}$, whereas $\rho_{\{5\}}(\sigma)$ switches both $0$ and
$1$. Vertex $0$ is not strictly switchable at $\sigma$ and yet
$w^\ast=(\tfrac34,\tfrac34,\tfrac34,\tfrac12,\tfrac12,\tfrac34,0,\tfrac34)$
prescribes $0\mapsto5$. Consequently $\rho_{\{5\}}$ is neither
residue-blind nor a switching rule in the sense of \Cref{def:rule}, and
\Cref{thm:normalform-barrier} does not apply to it.
\end{proposition}

\begin{proof}
All the numbers are immediate. Under $\sigma$, vertex $6$ pays $0$, so
$\val_\sigma(1)=0$ and $\val_\sigma(5)=0$; vertex $3$ pays
$\frac12(1+0)=\frac12$, so $\val_\sigma(4)=\frac12$ and
$\val_\sigma(0)=\frac12$; vertex $2$ pays $\frac12(\frac12+1)=\frac34$,
so $\val_\sigma(7)=\frac34$. At vertex $0$ the alternative successor $5$
has $\val_\sigma(5)=0<\frac12$, so $0\notin S_\sigma$; at vertex $1$ the
alternative $7$ has $\frac34>0$, so $S_\sigma=\{1\}$. Freeing $5$ and
freezing the rest, the residual reads
$\val(1)=\max(x(6),x(7))=\frac34$, $\val(5)=\val(1)=\frac34$ and
$\val(0)=\max(x(4),\val(5))=\max(\frac12,\frac34)=\frac34$, so the
optimal residual strategy sends $0$ to $5$ and $1$ to $7$. Optimality of
$0\mapsto5$ in $G^{\dagger}$ follows from $w^\ast$, which is checked
against $T$ directly.
\end{proof}
...
$|B|=1$ escapes it, and does so measurably --- on the normalised ladders
$N(L_n)$, $n=2,\dots,6$, the rule that tries every single $c$ and keeps
the best takes $1,2,2,2,3$ rounds where all-switches takes $n$.
...
value-preserving normalisation adding $2N$ average vertices with parallel
edges, so no rule of that kind runs in polynomially many rounds unless
all-switches already does (\Cref{thm:normalform-barrier}) --- unless it
first deletes the padding, which \Cref{rem:barrier-scope} explains is
exactly what the barrier leaves open.
