...
| `oracle-barrier` | 05:19 UTC | new-barrier | in progress: the route's BC(2,5), BC(3,5), BC(4,5) games loaded, $w^*$, the gap $2^{-s}$ at $v_0$ and the seeds confirmed; the root's own Z-seeded own-successor hybrid and `def:ratio` (both clauses) running on them (`bc_verify.py`) | pending | pending both audits |
