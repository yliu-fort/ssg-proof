...
| `few-denominator-stall` | strict-progress (re-run in `round16b`) | $\mathrm{CV}(e,s)$: a one-player family with $D(G)=2^{s}$ constant and one value-distinguishing Max vertex of gap exactly $1/D$, at which --- and at every controlled vertex --- the Z-seeded own-successor hybrid is silent for $2^{\Omega(N)}$ rounds by an explicit convex-certificate chain (`thm:cv-lower`; the correctness audit verified the certificate entrywise at 29 instances up to $N=62$, and I re-verified the values, $D$, the gap and the hybrid's firing round). Answers the question left open after `prop:fv-stall`. **Struck:** the claim that the ratio calculus M6 is also stalled --- the route's driver tested only clause (i) of `rem:own-successor`; with both clauses M6 fires at $v_0$ at rounds $12,22,39,74$ ($s=4$), found by the audit and reproduced here (`cv_ratio.py`). The significance audit's point that the wedge already stalls the hybrid at a value-distinguishing vertex is in `rem:cv`: what CV adds is the coarse gap. Four minor repairs (the width law $\lambda^{k-1}/2$, the M2 growth crossover, the grid $2^{-s}\mathbb Z$, one sentence in step (g)). |
