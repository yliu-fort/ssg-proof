export const meta = {
  name: 'ssg-round16b',
  description: 'Round 16b (completion) on the SSG value problem: the few-denominator-stall route with its two audits, and the four audits that hit the session limit (hk-doubling x2, one-player-howard correctness, fresh-16-alg correctness): 10 routes on Opus 5 against the post-round-15 frontier (176 pp), each adversarially audited twice on Opus 5, plus seven adversarial audits of the round-15-integrated parts of frontier.tex on Opus 5',
  phases: [
    { title: 'Routes' },
    { title: 'Audit' },
    { title: 'Paper audit' },
...
log(`Round 16b: route few-denominator-stall + 2 audits; 4 replacement audits from saved results (${args.audits.map(a => a.route + ':' + a.lens).join(', ')}).`)

const fd = ROUTES.find((r) => r.key === 'few-denominator-stall')
...
const extraWork = parallel(args.audits.map((a) => () => {
  const r = ROUTES.find((x) => x.key === a.route)
  const L = AUDIT_LENSES.find((x) => x.key === a.lens)
  return agent(L.prompt(r, a.text), { label: `audit:${a.route}:${a.lens}`, phase: 'Audit', schema: AUDIT_SCHEMA, model: 'opus' })
    .then((res) => (res ? { route: a.route, lens: a.lens, audit: res } : null))
}))

const [routes, extra] = await Promise.all([routeWork, extraWork])
log(`Round 16b complete: ${routes.filter(Boolean).length}/1 route; ${extra.filter(Boolean).length}/${args.audits.length} replacement audits.`)
return { round: '16b', routes: routes.filter(Boolean), extra: extra.filter(Boolean) }
