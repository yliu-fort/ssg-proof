...
TARGET = [int(x) for x in sys.argv[6].split(',')]
NS = 1 << m
T = np.array([[1 if (TARGET[sg] >> i) & 1 else -1 for i in range(m)] for sg in range(NS)], dtype=np.int8).reshape(-1)
...
while time.time() - t0 < secs:
    ...
    if (c, s) < bestc:
        ...
        if c == 0:
            ...
            json.dump(dict(m=m, k=k, den=den, A=Ai.tolist(), b=bi.tolist(), target=TARGET), open(f'G_m{m}_k{k}_den{den}_s{seed}.json', 'w'))
            break
    if it % 20000 == 0:
