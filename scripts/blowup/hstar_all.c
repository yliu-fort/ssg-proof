...
static int complete(int p){
    if(p==nrest){ if(is_acyclic()){ n_found++; printf("COMPLETION"); for(int v=0;v<NV;v++) printf(" %d",s[v]); printf("\n"); } return 0; }
    int v=rest[p];
    for(int t=0;t<NV;t++){
        if(used[t]) continue;
        int ok=1;
...
int main(int argc,char**argv){
    M = atoi(argv[1]); int L = atoi(argv[2]);
    NV=1<<M; FULL=NV-1; sig[0]=0;
    MODE=1; TARGET=L; n_seq_at_L=0; found=0; n_completion_tries=0;
    rec(0);
    printf("m=%d L=%d sequences=%lld completion_attempts=%lld acyclic_completions=%lld\n", M, L, n_seq_at_L, n_completion_tries, n_found);
    return 0;
}
