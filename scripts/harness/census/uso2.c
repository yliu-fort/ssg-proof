...
static void record(void){
    ...
    if(canon){ cls_auso++; h_cls[h]++; }
    if(h>best_h){ best_h=h; n_best=0; memcpy(best_map,s,sizeof s); }
    if(h==best_h) n_best++;
    if(holt_klee()){
        n_auso_hk++; h_hist_hk[h]++;
        if(canon){ cls_hk++; h_cls_hk[h]++; }
        if(h>best_h_hk){ best_h_hk=h; n_best_hk=0; memcpy(best_map_hk,s,sizeof s); }
        if(h==best_h_hk) n_best_hk++;
    }
}
